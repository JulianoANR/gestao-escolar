...
class User extends Authenticatable
{
    ...
    public function getFormatedSalasAttribute()
    {
        ...
        foreach ($salas as $sala) {
            if ($sala->serie == 'bercario_1') {
                $sala->serie = 'Berçário 1';
            } elseif ($sala->serie == 'bercario_2') {
                $sala->serie = 'Berçário 2';
            } elseif ($sala->serie == 'fase_1') {
                $sala->serie = 'Fase 1';
            } elseif ($sala->serie == 'fase_2') {
                $sala->serie = 'Fase 2';
            } elseif ($sala->serie == 'maternal_1') {
                $sala->serie = 'Maternal 1';
            } elseif ($sala->serie == 'maternal_2') {
                $sala->serie = 'Maternal 2';
            } elseif ($sala->serie == 'eja1_t1') {
                $sala->serie = 'Eja1 - T1';
            } elseif ($sala->serie == 'eja1_t2') {
                $sala->serie = 'Eja1 - T2';
            } elseif ($sala->serie == 'eja1_t3') {
                $sala->serie = 'Eja1 - T3';
            } elseif ($sala->serie == 'eja1_t4') {
                $sala->serie = 'Eja1 - T4';
            } elseif ($sala->serie == 'eja2_t1') {
                $sala->serie = 'Eja2 - T1';
            } elseif ($sala->serie == 'eja2_t2') {
                $sala->serie = 'Eja2 - T2';
            } elseif ($sala->serie == 'eja2_t3') {
                $sala->serie = 'Eja2 - T3';
            } elseif ($sala->serie == 'eja2_t4') {
                $sala->serie = 'Eja2 - T4';
            }
            if ($sala->nivel_ensino == 'educ_infantil') {
                $sala->nivel_ensino = 'Educação Infantil';
            } elseif ($sala->nivel_ensino == 'fundamental_1') {
                $sala->nivel_ensino = 'Fundamental 1';
            } elseif ($sala->nivel_ensino == 'fundamental_2') {
                $sala->nivel_ensino = 'Fundamental 2';
            } elseif ($sala->nivel_ensino == 'eja') {
                $sala->nivel_ensino = 'Educação de Jovens e Adultos';
            }
        }
